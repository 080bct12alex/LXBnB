# Setting Up and Running the LXBnb : HOTEL Booking App  on  Local Machine
...
2. **MongoDB Setup**: 
    ...
    - Once set up, obtain  MongoDB connection string and add it to the `MONGODB_CONNECTION_STRING` variable in your `.env` files.
    - For the `.env.e2e` setup see "running automated tests" below

3. **Cloudinary Setup**:
    - Create an account at [Cloudinary](https://cloudinary.com/).
    - Navigate to  dashboard to find cloud name, API key, and API secret.
    - Add these details to the respective `CLOUDINARY_*` variables in your `.env` files.

4. **Stripe Setup**:
    - Sign up for a Stripe account at [Stripe](https://stripe.com/).
    - Find  API keys in the Stripe dashboard.
    - Add  Stripe API key to the `STRIPE_API_KEY` variable in 
     `.env` files.
  
5. **JWT_SECRET_KEY**:
    - This just needs to be any long, random string or  google "secret key generator"  or bash .

6. **Frontend URL**:
    - The `FRONTEND_URL` should point to the URL where  frontend application is running (typically `http://localhost:3000` if  running  in locally).
...
5. **VITE_API_BASE_URLL**:
    - The `VITE_API_BASE_URL` should point to the URL where backend application is running (typically `http://localhost:7000` if  running in locally).
...
2. **Frontend**:
    ...
    - The application should now be running on `http://localhost:5173` but verify this in  command line terminal  
...
2. **Importing Test Data into MongoDB**:
    ...
    - Import `data` folder at the root, which includes JSON files for a test user and a test hotel.  into  MongoDB collections to quickly set up test data.
    ...
    - **Import User Data**:
        - Navigate to the `users` collection within database. Create it if it doesn't exist
        - Click on the "Add Data" button and select "Import File".
        - Browse to the location of  `test-users.json` file and select it.
    ...
    -  **Locate the Test Hotel File**:
        - Navigate to the `hotels` collection within  database. Create it if it doesn't exist
        - Repeat the import process as did for the user data, but this time select the `test-hotel.json` file.
